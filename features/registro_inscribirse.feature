Feature: Como usuario quiero poder realizar un registro a un evento al dar clic a "inscribirse" dentro de la pestaña de evento
La aplicación permite realizar un registro donde los usuarios insertan sus datos y dan click en "inscribirse" para poder apartar su lugar en eventos de la universidad ITESO.
...
Scenario: The one where si el registro no se realiza correctamente, aparecerá una ventana que indique "registro no realizado, intente de nuevo"
    Given di click a “inscribirse”
    When se realiza el registro pero ocurre un error
    Then No se capturó y aparece una ventana que indica "registro no realizado, intente de nuevo"

Scenario: The one where si el registro no se realiza correctamente (no se da click en "inscribirse"), aparecerá una ventana que indique "registro no realizado, intente de nuevo"
    Given no di click a “inscribirse”
