Feature: Como usuario, quiero tener mi propio sistema de registro, en donde se guarden mis datos: expediente, carrera y tipo de evento al que asistiré
    La aplicación nos permite tener un registro donde los usuarios insertan sus datos completos (expediente, carrera y tipo de evento que asistirá) para poder apartar su lugar en diferentes eventos de la universidad ITESO.

Scenario: The one where se tiene que contar con una cuenta
    Given cuento con una cuenta
    When ingreso a la página
    Then me permite ingresar sesión

Scenario: The one where inicia sesión en la aplicación
    Given ingreso a la página de Civiteso
    When coloco mi usuario y contraseña
    Then inicio sesión en la aplicación

Scenario: The one where no puede iniciar sesión
    Given ingreso a la página de Civiteso
...
Scenario: The one where si no se registran todos los datos (expediente, carrera y tipo de evento) no se podrá realizar el registro
    Given seleccioné la pestaña de registro
...
Scenario: The one where i el registro no se realiza correctamente (no se ingresan datos correspondientes), aparecerá una ventana que indique "registro no realizado, intente de nuevo"
...
Scenario: The one where Si se aceptan los términos y condiciones, se podrá hacer el registro
